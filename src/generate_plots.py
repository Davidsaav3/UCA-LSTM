...
# 1️⃣ Serie temporal con anomalías reales y detectadas
plt.figure(figsize=(14,6))
plt.plot(df[target_col], alpha=params["alpha"], label="Serie")
plt.scatter(df.index[df["is_anomaly_real"]==1], df[target_col][df["is_anomaly_real"]==1],
            color=params["colors_anomaly_real"], marker=params["marker_anomaly_real"], label="Anomalias reales")
plt.scatter(df.index[df["is_anomaly_detected"]==1], df[target_col][df["is_anomaly_detected"]==1],
            color=params["colors_anomaly_detected"], marker=params["marker_anomaly_detected"], label="Anomalias detectadas", facecolors="none")
